...
def remove_repeated_events(event_list_values, key):
    ...
    # Initialize a set to keep track of unique values for the given key
    unique_values_set = set()

    # Initialize a list to store unique events
    unique_events = []

    for event in event_list_values:
        value = event.get(key)  # Accessing value using the key
